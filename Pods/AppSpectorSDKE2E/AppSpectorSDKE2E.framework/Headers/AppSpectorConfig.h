...
/**
 Builds config with all monitors enabled and allows to provide public key for e2e encryption
 
 @param apiKey NSString key used to access application instance. You can get one on settings page after creating app on the frontend.
 @param publicKey NSString key used to establish encrypted sessions, could be found in app settings on frontend.
...
+ (instancetype)configWithAPIKey:(NSString *)apiKey
                       publicKey:(NSString *)publicKey
                      monitorIDs:(NSSet <NSString *> *)monitorIDs;
